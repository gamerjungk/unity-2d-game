...
namespace CiDy
{
	...
	public class CiDyCell : MonoBehaviour
	{
		...
		public bool enableClutter = true;//Default to True as most people want clutter objects along there SideWalks.
				...
				for (int j = 0; j < pointsList[i].Count - 1; j++)
				{
					...
					if (stepSpace > 0)
					{
						for (int k = 0; k < stepSpace; k++)
						{
							...
							if (clutterObjects.Length > 0 && enableClutter)
							{
								//Place Clutter
								if (cluttersCurDist >= pathClutterSpacing)
								{
									...
									if (!cantPlace)
									{
										...
										{
											//get Current Seed
											int curSeed = seedValue;//Save
											//set to
											Random.InitState(r.Next());
											objPlace = Random.Range(0, clutterObjects.Length - 1);
